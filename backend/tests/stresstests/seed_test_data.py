...
def setup_test_environment(count):
    ...
    if login_response.status_code != 200:
        logger.error("Admin-Login fehlgeschlagen. Läuft das Backend?")
        return
    ...
    logger.info("--- 2. Stammdaten anlegen ---")
    ...
    if res_profil.status_code in [200, 201]:
        ...
        logger.info("Kassenprofil erstellt (ID: %s)", kassenprofil_id)
    else:
        logger.warning("Kassenprofil konnte nicht erstellt werden: %s", res_profil.text)
    ...
    if res_veranstaltung.status_code in [200, 201]:
        logger.info("Veranstaltung erfolgreich erstellt (ID 1).")
    else:
        logger.error("Fehler bei Veranstaltung: %s", res_veranstaltung.text)
    ...
    if res_artikel.status_code in [200, 201]:
        logger.info("Artikel erfolgreich erstellt (ID 1).")
    else:
        logger.warning("Fehler bei Artikel (vielleicht schon vorhanden?): %s", res_artikel.text)
    ...
    if res_zahlung.status_code in [200, 201]:
        logger.info("Zahlungsmethode erstellt (ID 1).")

    # 6. User anlegen
    logger.info("--- 3. Seeding von %s Benutzern ---", count)
    for i in range(1, count + 1):
        ...
        try:
            ...
            if res_user.status_code not in [200, 201]:
                logger.error("Fehler bei Benutzer %s: %s - %s", i, res_user.status_code, res_user.text)
        except requests.exceptions.RequestException as e:
            logger.error("Verbindungsfehler bei User %s: %s", i, e)
            break

    logger.info("Seeding abgeschlossen! Der Locust-Test kann gestartet werden.")
